...
void de::andruav_servers::CAndruavParser::parseCommand(
    const std::string &sender_party_id, const int &command_type,
    const Json_de &jsonMessage) {
#ifdef DDEBUG_PARSER
  std::cout << __PRETTY_FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: parseCommand "
  ...
  if (validateField(jsonMessage, ANDRUAV_PROTOCOL_MESSAGE_PERMISSION,
  ...
  if ((validateField(jsonMessage, ANDRUAV_PROTOCOL_SENDER,
                     Json_de::value_t::string)) &&
      (jsonMessage[ANDRUAV_PROTOCOL_SENDER].get<std::string>().compare(
           ANDRUAV_PROTOCOL_SENDER_COMM_SERVER) ==
       0)) { // permission is not needed if this command sender is the
             // communication server not a remote GCS or Unit.
    is_system = true;
  }

  // get message command details
  const Json_de &msg_cmd = jsonMessage.contains(ANDRUAV_PROTOCOL_MESSAGE_CMD)
                               ? jsonMessage[ANDRUAV_PROTOCOL_MESSAGE_CMD]
                               : Json_de();

  // if unit is new then ask for details.
  if ((command_type != TYPE_AndruavMessage_ID) && (unit_info.is_new == true)) {
    de::andruav_servers::CAndruavFacade::getInstance().API_requestID(
        sender_party_id);

    /*
        DONOT add return here unless system requires more security.
        You cannot receive messages except via Communication Server from units
       that are logged into the system so it should be secure. if you enable the
       return the following issue may happen: 1- GCS receives ID messages from
       the unit. 2- GCS send asking for mission & other info 3- unit will ignore
       these messages until it receives a MSG_ID from the WEB. 4- WebClient
       sending messages is stateless in general.
    */
  ...
  case TYPE_AndruavMessage_ID: {
    ...
    */
    ...
    if (!validateField(command, "UD", Json_de::value_t::string))
      return;
    if (!validateField(command, "DS", Json_de::value_t::string))
      return;

    unit_info.unit_name = command["UD"].get<std::string>();
    unit_info.description = command["DS"].get<std::string>();
    if (command.contains("VR") == true)
      unit_info.is_video_recording = command["VR"].get<int>();
    if (command.contains("FI") == true)
      unit_info.use_fcb = command["FI"].get<bool>();
    if (command.contains("SD") == true)
      unit_info.is_shutdown = command["SD"].get<bool>();
    if (command.contains("GM") == true)
      unit_info.gps_mode = command["GM"].get<int>();
    if (command.contains("AR") == true)
      unit_info.armed_status = command["AR"].get<int>();
    if (command.contains("FL") == true)
      unit_info.is_flying = command["FL"].get<bool>();
    if (command.contains("AP") == true)
      unit_info.autopilot = command["AP"].get<int>();
    if (command.contains("FM") == true)
      unit_info.flying_mode = command["FM"].get<int>();
    if (command.contains("B") == true)
      unit_info.is_gcs_blocked = command["B"].get<bool>();
    if (command.contains("x") == true)
      unit_info.is_flashing = command["x"].get<bool>();
    if (command.contains("y") == true)
      unit_info.is_whisling = command["y"].get<bool>();
    if (command.contains("b") == true)
      unit_info.is_tracking_mode = command["b"].get<bool>();
    if (command.contains("z") == true)
      unit_info.flying_last_start_time = command["z"].get<long long>();
    if (command.contains("a") == true)
      unit_info.flying_total_duration = command["a"].get<long long>();
    if (command.contains("p") == true)
      unit_info.permission = command["p"].get<std::string>();

    if (command.contains("C") == true)
      unit_info.manual_TX_blocked_mode = command["C"].get<int>();

    if (command.contains("n") == true)
      unit_info.swarm_follower_formation = command["n"].get<int>();
    if (command.contains("o") == true)
      unit_info.swarm_leader_formation = command["o"].get<int>();
    if (command.contains("q") == true)
      unit_info.swarm_leader_I_am_following = command["q"].get<std::string>();

    // if is for backward compatibility
    if (command.contains("T") == true)
      unit_info.unit_sync_time = command["T"];
    unit_info.last_access_time = get_time_usec();

    // std::string res = unit_info.is_new?"New":"OLD" ;
    // std::cout << _INFO_CONSOLE_TEXT << "TYPE_AndruavMessage_ID: " <<
    // unit_info.is_new << _SUCCESS_CONSOLE_TEXT_ <<  msg_cmd.dump() <<
    // _NORMAL_CONSOLE_TEXT_ << std::endl;
  ...
  case TYPE_AndruavMessage_Unit_Name: {
    ...
    */
    ...
    if (!validateField(command, "UN", Json_de::value_t::string))
      return;
    if (!validateField(command, "DS", Json_de::value_t::string))
      return;
    ...
    cLocalConfigFile.addStringField("unitDescription",
                                    unit_info.description.c_str());
    cLocalConfigFile.apply();
    if ((command.contains("PR") == true) &&
        (command["PR"].get<bool>() == true)) {
      ...
      cLocalConfigFile.apply();
      /*
        Do not change party_id this will make it unstable because
        CommunicationServer expects current partyID Change will take effective
        after reboot or server disconnection. Also if server is asking for fixed
        UDP port then communication server will allocate the port for the old
        one.
      */
      // unit_info.party_id = party_id;   << do not uncomment.
    }
    de::andruav_servers::CAndruavFacade::getInstance().API_sendID(
        sender_party_id);
  } break;
  ...
  case TYPE_AndruavMessage_CONFIG_ACTION: {
    ...
    case CONFIG_ACTION_SHUT_DOWN: {
      ...
      std::thread([=]() {
        ...
        if (result != 0) {
          PLOG(plog::error)
              << "Failed to execute sh_shutdown.sh, return code: " << result;
          std::cout << _ERROR_CONSOLE_BOLD_TEXT_
                    << "Failed to execute sh_shutdown.sh, return code: "
                    << result << _NORMAL_CONSOLE_TEXT_ << std::endl;
        } else {
          std::cout << _SUCCESS_CONSOLE_BOLD_TEXT_
                    << "Shutdown script executed successfully"
                    << _NORMAL_CONSOLE_TEXT_ << std::endl;
        }
      }).detach();
    ...
    case CONFIG_REQUEST_FETCH_CONFIG_TEMPLATE: {
      if (!jsonMessage.contains(ANDRUAV_PROTOCOL_SENDER))
        return;
      std::string sender =
          jsonMessage[ANDRUAV_PROTOCOL_SENDER].get<std::string>();

#ifdef DEBUG
      std::cout << std::endl
                << _INFO_CONSOLE_TEXT << "CONFIG_REQUEST_FETCH_CONFIG_TEMPLATE"
                << _NORMAL_CONSOLE_TEXT_ << std::endl;
#endif
      ...
      if (!file.is_open()) {
        ...
        std::cout << std::endl
                  << _ERROR_CONSOLE_BOLD_TEXT_ << "cannot read template.json"
                  << _NORMAL_CONSOLE_TEXT_ << std::endl;
        de::andruav_servers::CAndruavFacade::getInstance().API_sendErrorMessage(
            std::string(), 0, ERROR_3DR, NOTIFICATION_TYPE_ERROR,
            "cannot read template.json");
        Json_de empty_file_content_json = {};
        de::andruav_servers::CAndruavFacade::getInstance()
            .API_sendConfigTemplate(sender, module_key, empty_file_content_json,
                                    true);
        return;
      }
      ...
      de::andruav_servers::CAndruavFacade::getInstance().API_sendConfigTemplate(
          sender, module_key, file_content_json, true);
    } break;
  ...
  case TYPE_AndruavMessage_Communication_Line_Set: {
     ...
     * [w2]: for turning communication server LOCAL websocket on/off or leave as
     * is. [wd2]: duration of websocket off in seconds.
     */
    ...
    if (validateField(command, "ws", Json_de::value_t::boolean)) {
      ...
      de::andruav_servers::CAndruavCommServerManager::getInstance().turnOnOff(
          ws_on_off, ws_duration);
    }
    ...
    if (validateField(command, "w2", Json_de::value_t::boolean)) {
      ...
      de::andruav_servers::CAndruavCommServerManager::getInstance()
          .turnOnOffLocal(ws_on_off, ws_duration);
    }
  } break;
  ...
  case TYPE_AndruavMessage_LocalServer_ACTION: {
    ...
    if ((validateField(command, "u", Json_de::value_t::string)) &&
      ...
      de::andruav_servers::CAndruavCommServer::getInstance().turnOnOff(false,
                                                                       0);
      de::andruav_servers::CAndruavCommServerLocal::getInstance()
          .reconnectToCommServer("192.168.1.144", "9967", "my_key");
    }
  } break;
  ...
  case TYPE_AndruavMessage_Upload_DE_Mission: {
    ...
    */
    ...
    de::mission::CMissionManagerBase::getInstance().extractPlanModule(
        plan_object);

#ifdef DDEBUG
    std::cout << _INFO_CONSOLE_BOLD_TEXT
              << "DroneEngage Mission-Upload: " << plan_object.dump()
              << std::endl;
#endif
  } break;
  }
...
void de::andruav_servers::CAndruavParser::parseRemoteExecuteCommand(
    const std::string &sender_party_id, const Json_de &jsonMessage) {
#ifdef DDEBUG_PARSER
  std::cout << __PRETTY_FUNCTION__ << " line:" << __LINE__ << "  "
            << _LOG_CONSOLE_TEXT << "DEBUG: parseRemoteExecuteCommand "
            << _NORMAL_CONSOLE_TEXT_ << std::endl;
#endif
  ...
  if ((unit_info.is_new == true) &&
      (remote_execute_command != TYPE_AndruavMessage_ID)) {
    // the sender is a new unit so we ask for identification. ...

    de::andruav_servers::CAndruavFacade::getInstance().API_requestID(
        sender_party_id); // ask for identification in return.
    ...
    /*
        ...
        You cannot receive messages except via Communication Server from units
       that are logged into the system so it should be secure. ?If you enable
       the return the following issues may happen: 1- GCS receives ID messages
       from the unit. 2- GCS send asking for mission & other info 3- unit will
       ignore these messages until it receives a MSG_ID from the WEB. 4-
       WebClient sending messages is stateless in general.
    */
    // return ;
  }
  ...
  case TYPE_AndruavMessage_CameraList: {
    de::andruav_servers::CAndruavFacade::getInstance().API_sendCameraList(
        true, sender_party_id);
  } break;
  ...
  case RemoteCommand_TELEMETRYCTRL: {
    ...
    if (request_type != CONST_TELEMETRY_ADJUST_RATE)
      return;
