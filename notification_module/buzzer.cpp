...
bool CBuzzer::init (const std::vector<PORT_STATUS>& buzzer_pins)
{
    ...
    {
        std::cout << _SUCCESS_CONSOLE_TEXT_ << "Initalize Buzzer at GPIO " << std::to_string(pin.gpio_pin) << std::endl; 
...
void CBuzzer::update_pattern_to_play()
{
    ...
    if (m_flags.m_fcb_connected != STATUS::getInstance().is_online())
    {
        m_flags.m_fcb_connected = STATUS::getInstance().is_online();
...
void CBuzzer::on(const bool turn_on)
{
    ...
    hal_linux::CRPI_GPIO::getInstance().write(m_port_pins[0].gpio_pin, m_turn_on? LED_STATUS_ON : LED_STATUS_OFF);
   m_port_pins[0].status = turn_on?GPIO_ON:GPIO_OFF;
}
